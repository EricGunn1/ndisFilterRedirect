...
--*/
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "===> filterHelloWorld: DriverEntry.\n"));
    ...
    do
    {
        ...
        //
        FChars.SetOptionsHandler = NULL;// FilterRegisterOptions;
        FChars.AttachHandler = FilterAttach;
        FChars.DetachHandler = FilterDetach;
        FChars.RestartHandler = FilterRestart;
        FChars.PauseHandler = FilterPause;
        FChars.SetFilterModuleOptionsHandler = NULL;//FilterSetModuleOptions;
        FChars.OidRequestHandler = NULL; // FilterOidRequest;
        FChars.OidRequestCompleteHandler = NULL; //FilterOidRequestComplete;
        ...
        FChars.NetPnPEventHandler = NULL; //FilterNetPnPEvent;
        FChars.StatusHandler = NULL;// FilterStatus;
        FChars.CancelSendNetBufferListsHandler = NULL;// FilterCancelSendNetBufferLists;
        ...
        Status = filterHelloWorldRegisterDevice();
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "<===filterHelloWorld DriverEntry, Status = %8x\n", Status));
    return Status;
...
--*/
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "===> filterHelloWorld: FilterRegisterOptions.\n"));
...
--*/
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "===> filterHelloWorld FilterAttach: NdisFilterHandle %p\n", NdisFilterHandle));
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "===> filterHelloWorld FilterAttach: AttachParameters->MiniportMediaType %x\n", AttachParameters->MiniportMediaType));
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "<=== filterHelloWorld FilterAttach:    Status %x\n", Status));
...
--*/
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "===> filterHelloWorld FilterPause: FilterInstance %p\n", FilterModuleContext));
    //DEBUGP(DL_TRACE, "===>filterHelloWorld FilterPause: FilterInstance %p\n", FilterModuleContext);
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "<=== filterHelloWorld FilterPause:  Status %x\n", Status));
...
--*/
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "===> filterHelloWorld FilterRestart:   FilterModuleContext %p\n", FilterModuleContext));
    ...
    if (Status != NDIS_STATUS_SUCCESS)
    {
        ...
        //
        PWCHAR              ErrorString = L"filterHelloWorld";
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "<=== filterHelloWorld FilterRestart:  FilterModuleContext %p, Status %x\n", FilterModuleContext, Status));
...
--*/
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "===> filterHelloWorld FilterDetach:    FilterInstance %p\n", FilterModuleContext));
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "<=== filterHelloWorld FilterDetach Successfully\n"));
...
--*/
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "===> filterHelloWorld FilterUnload\n"));
    //DEBUGP(DL_TRACE, "===>FilterUnload\n");
    //DbgBreakPoint();
    //
    // Should free the filter context list
    //
    filterHelloWorldDeregisterDevice();
    NdisFDeregisterFilterDriver(FilterDriverHandle);
...
--*/
{
    ...
    //KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "===> filterHelloWorld FilterOidRequest: Request %p.\n", Request));
    ...
    //KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "<=== filterHelloWorld FilterOidRequest: Status %8x.\n", Status));
...
--*/
{
    ...
    //KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "===> filterHelloWorld FilterOidRequestComplete, Request %p.\n", Request));
    ...
    //KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "<=== filterHelloWorld FilterOidRequestComplete.\n"));
...
--*/
{
    ...
    //KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "===> filterHelloWorld FilterStatus, IndicateStatus = %8x.\n", StatusIndication->StatusCode));
    ...
    //KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "<=== filterHelloWorld FilterStatus.\n"));
...
--*/
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "===> filterHelloWorld FilterDevicePnPEventNotify: NetPnPEvent = %p.\n", NetDevicePnPEvent));
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "<=== filterHelloWorld FilterDevicePnPEventNotify\n"));
...
typedef struct _MY_ETHERNET_HEADER {
    UCHAR DestinationMAC[6]; // Destination MAC address
    UCHAR SourceMAC[6];      // Source MAC address
    USHORT EtherType;        // Ethernet Type or Length field
} MY_ETHERNET_HEADER, * PMY_ETHERNET_HEADER;

typedef struct _MY_IPVFOUR_HEADER {
    UCHAR VersionAndIHL;      // Version and Internet Header Length
    UCHAR TypeOfService;      // Type of Service
    USHORT TotalLength;       // Total Length
    USHORT Identification;    // Identification
    USHORT FlagsAndOffset;    // Flags and Fragment Offset
    UCHAR TimeToLive;         // Time to Live
    UCHAR Protocol;           // Protocol
    USHORT HeaderChecksum;    // Header Checksum
    UINT SourceAddress;     // Source IP Address
    UINT DestinationAddress;// Destination IP Address
    // Additional fields (if any)
} MY_IPVFOUR_HEADER, * PMY_IPVFOUR_HEADER;

typedef struct _MY_IPVSIX_HEADER {
    UINT VersionAndTrafficClassAndFlowLabel;  // Version, Traffic Class, and Flow Label
    USHORT PayloadLength;                       // Payload Length
    UCHAR  NextHeader;                          // Next Header
    UCHAR  HopLimit;                             // Hop Limit
    USHORT  SourceAddress[8];                    // Source IPv6 Address
    USHORT  DestinationAddress[8];               // Destination IPv6 Address
    // Additional fields (if any)
} MY_IPVSIX_HEADER, * PMY_IPVSIX_HEADER;

typedef struct _MY_TCP_HEADER {
    USHORT SourcePort;
    USHORT DestinationPort;
    UINT SequenceNumber;
    UINT AcknowledgmentNumber;
    USHORT DataOffsetAndFlags;
    USHORT WindowSize;
    USHORT Checksum;
    USHORT UrgentPointer;
    // Add additional fields as needed
} MY_TCP_HEADER, *PMY_TCP_HEADER;

typedef struct _MY_UDP_HEADER {
    USHORT SourcePort;
    USHORT DestinationPort;
    USHORT Length;
    USHORT Checksum;
    // Add additional fields as needed
} MY_UDP_HEADER, * PMY_UDP_HEADER;

typedef struct _MY_PACKET_HEADER {
    MY_ETHERNET_HEADER EthernetHeader;
    MY_IPVFOUR_HEADER IP4Header;
    MY_TCP_HEADER TCPHeader;
}MY_PACKET_HEADER, *PMY_PACKET_HEADER;

typedef struct _MY_UDP_PSEUDOHEADER {
    UINT SourceAddress;     // Source IP Address
    UINT DestinationAddress;// Destination IP Address
    UCHAR zeros;
    UCHAR Protocol;           // Protocol
    USHORT TotalLength;       // Total Length
    // Additional fields (if any)
} MY_UDP_PSEUDOHEADER, * PMY_UDP_PSEUDOHEADER;





void parseMACAddress(PMY_ETHERNET_HEADER EthHeader) {
    INT x;
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, EthHeader->DestMac = "));
    for (x = 0; x < 6; x++) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "%x-", EthHeader->DestinationMAC[x]));
    }
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "\b\n"));

    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, EthHeader->SourceMac = "));
    for (x = 0; x < 6; x++) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "%x-", EthHeader->SourceMAC[x]));
    }
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "\b\n"));

    //KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, EthHeader->EtherType =  %x\n", EthHeader->EtherType));
}


void parseIPHeader(PMY_IPVFOUR_HEADER IP4Header, PMY_IPVSIX_HEADER IP6Header) {
    USHORT IP6segment;
    UINT IP4segment;
    PUINT PIP4segment;
    USHORT version;
    ...
    version = (IP4Header->VersionAndIHL >> 4) & 0x0F;
    if (version == 4) {
        //KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, version = %x\n", version));
        PIP4segment = &IP4Header->SourceAddress;
        KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, IP4Header->SourceAddress = "));
        for (x = 0; x < 4; x++) {
            IP4segment = *PIP4segment;
            IP4segment = (IP4segment >> (8 * x)) & 0xff;
            KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "%d.", IP4segment));
            IP4segment++;
        }
        KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "\n"));


        PIP4segment = &IP4Header->DestinationAddress;
        KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, IP4Header->DestinationAddress = "));
        for (x = 0; x < 4; x++) {
            IP4segment = *PIP4segment;
            IP4segment = (IP4segment >> (8 * x)) & 0xff;
            KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "%d.", IP4segment));
        ...
        version = (IP6Header->VersionAndTrafficClassAndFlowLabel >> 28) & 0x0F;
        if (version == 6) {
            KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, version = %x\n", version));
            KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, IP6Header->SourceAddress = "));
            for (x = 0; x < 8; x++) {
                IP6segment = IP6Header->SourceAddress[x];
                IP6segment = (IP6segment >> 8) | (IP6segment << 8);
                KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "%x:", IP6segment));
            }
            KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "\n"));
            KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, IP6Header->DestinationAddress = "));
            for (x = 0; x < 8; x++) {
                IP6segment = IP6Header->DestinationAddress[x];
                IP6segment = (IP6segment >> 8) | (IP6segment << 8);
                KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "%x:", IP6segment));
            }
            KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "\n"));
            ...
            KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, Unrecognized version\n"));
            return;
        }
    }
}

void parseTCPHeader(PMY_TCP_HEADER TCPHeader) {
    USHORT dataOffset;
    USHORT sourcePort;
    USHORT destPort;

    dataOffset = TCPHeader->DataOffsetAndFlags;
    dataOffset = (dataOffset >> 12);
    sourcePort = TCPHeader->SourcePort;
    destPort = TCPHeader->DestinationPort;
    sourcePort = (sourcePort << 8) | (sourcePort >> 8);
    destPort = (destPort << 8) | (destPort >> 8);

    //KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, dataOffsetAndFlags = %x\n", TCPHeader->DataOffsetAndFlags));
    //KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, dataOffset = %x\n", dataOffset));
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, SourcePort = %d\n", sourcePort));
    KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld projectRedirect, DestPort = %d\n", destPort));

}

void parseUDPHeader(PMY_UDP_HEADER UDPHeader) {
...
INT counter = 0;
USHORT calculateChecksum(PUSHORT dnsData, UINT dnsDataLegnth) {
    UINT sum = 0;



    while (dnsDataLegnth > 1) {
        sum += *dnsData;
        dnsData++;
        dnsDataLegnth -= 2;
    }
...
USHORT findCNN(PMDL currMdl, PVOID mdlSystemVa) {
    ...
    PUINT pMdlTargetString = &cnnString;
...
VOID redirectIPV4(PVOID storageBuffer, PNET_BUFFER currNb) {
    ...
    PMY_UDP_PSEUDOHEADER pPseudoheader = NULL;
    UCHAR protocol;
    UINT sourceAddress;
...
VOID redirectIPV6(PVOID storageBuffer, PNET_BUFFER currNb) {
    PMY_ETHERNET_HEADER EthHeader = NULL;
    PMY_IPVSIX_HEADER IP6Header = NULL;
    PMY_UDP_HEADER UDPHeader = NULL;
    PVOID placeholder;

    placeholder = currNb;
    EthHeader = (PMY_ETHERNET_HEADER)storageBuffer;
    IP6Header = (PMY_IPVSIX_HEADER)((char*)EthHeader + sizeof(MY_ETHERNET_HEADER));
    UDPHeader = (PMY_UDP_HEADER)((char*)EthHeader + sizeof(MY_ETHERNET_HEADER) + sizeof(MY_IPVSIX_HEADER));
    
    if (UDPHeader->DestinationPort == 0x3500) {
...
--*/
{
    ...
    while (bFalse);
    //KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "<=== filterHelloWorld SendNetBufferList.\n"));
...
--*/
{
    ...
    //NdisFreeCloneNetBufferList(NetBufferLists, NDIS_CLONE_FLAGS_RESERVED);
    //KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "<=== filterHelloWorld ReturnNetBufferLists.\n"));
...
VOID acceptIPV4(PNET_BUFFER currNb) {
    ...
    PMY_IPVFOUR_HEADER IP4Header = NULL;
    PMY_UDP_HEADER UDPHeader = NULL;
    PMY_UDP_PSEUDOHEADER pPseudoheader;
    UINT sourceAddress;
    UINT destinationAddress;
    UCHAR protocol;
    USHORT totalLength;
...
VOID acceptIPV6(PNET_BUFFER currNb) {
    PMY_ETHERNET_HEADER EthHeader = NULL;
    PMY_IPVSIX_HEADER IP6Header = NULL;
    PMY_UDP_HEADER UDPHeader = NULL;
    PMDL currMdl = NULL;
    ...
    if (UDPHeader->SourcePort == 0x3500) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld redirectIPV6, IPv6 DNS received\n"));
        //if (counter < 10) {
        //    DbgBreakPoint();
        //    counter++;
        //}
    ...
    while (currNb != NULL) {
        ...
        if (IPversion == 4) {
            ...
            if (EthHeader->EtherType != 0x608) {
                KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "filterHelloWorld acceptRedirect, something else received\n"));
                DbgBreakPoint();
            }
        }
        currNb = NET_BUFFER_NEXT_NB(currNb);
...
FilterReceiveNetBufferLists(
    ...
    /*
    Receive Flag Info:
    NDIS consistently passing 0x00000401

    NDIS_RECEIVE_FLAGS_DISPATCH_LEVEL = 001
    Specifies that the current IRQL is DISPATCH_LEVEL.

    NDIS_RECEIVE_FLAGS_RESOURCES = 002
    ...
    NDIS_RECEIVE_FLAGS_PERFECT_FILTERED = 400
    Specifies that all the NET_BUFFER_LIST structures in the list at NetBufferLists include only data that matches the packet filter and multicast address list that are assigned to the miniport adapter.
    */
...
//#endif
    ...
    //KdPrintEx((DPFLTR_IHVDRIVER_ID, 0xffffffff, "<=== filterHelloWorld ReceiveNetBufferList: Flags = %8x.\n", ReceiveFlags));
